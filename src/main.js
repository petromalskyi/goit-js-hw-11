...
function handleSubmit(event) {
  ...
  listEl.innerHTML = '';
  const userEntered = event.currentTarget.elements.name.value;
...
function getGallery(userEntered) {
  ...
  return fetch(
    `${BASE_URL}/?key=${API_KEY}&q=${userEntered}&image_type=photo&orientation=horizontal&safesearch=true&per_page=200`,
  ).then(resp => {
    if (!resp.ok) {
